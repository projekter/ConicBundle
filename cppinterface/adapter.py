...
f_total = open("../../ConicBundle.jl/src/cppinterface/ConicBundle_cpp.jl", "w+")
f_total_c = open("cb_cppinterface.cxx", "w+")
f_classes = open("../../ConicBundle.jl/src/cppinterface/cb_classes.jl", "w+")
f_doc = open("../../ConicBundle.jl/docs/src/cppref.md", "w+")
...
for filename, classes in adapt.items():
    for curfile in glob.glob(f"..\\**\\{filename}", recursive=True):
...
""", "", data, 0, re.VERBOSE)
            ...
            f_curclass_c = open(nameprefix[:-1] + ".cpp", "w+")
            f_curclass_jl = open("../../ConicBundle.jl/src/cppinterface/" + nameprefix[:-1] + ".jl", "w+")
